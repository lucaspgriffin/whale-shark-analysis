...
# Apply time conversion to all datasets
time_datasets <- list(general22, general16, boat22, boat16, swim22, swim16)
for(i in seq_along(time_datasets)) {
  time_datasets[[i]]$V_LENGTH <- sapply(time_datasets[[i]]$V_LENGTH, convert_time_format)
}
list2env(setNames(time_datasets, c("general22", "general16", "boat22", "boat16", "swim22", "swim16")), envir = .GlobalEnv)
```
...
logbook_results <- data.frame(
  ...
  Min_2016 = min(infield16_boat$NO_BOAT),
  Max_2016 = max(infield16_boat$NO_BOAT),
  Min_2022 = min(infield22$Boat_total),
...
# Concurvity check
knitr::kable(round(concurvity(m_gam_boat, full = TRUE), 3), caption = "Concurvity - Boat Model")
...
pred_boat <- predict.gam(model_2, newdata = grid_boat, type = "link", se.fit = TRUE, exclude = "s(DATE)")
...
inverse_link <- family(model_2)$linkinv
...
# Concurvity check
knitr::kable(round(concurvity(m_gam_swim, full = TRUE), 3), caption = "Concurvity - Swimmer Model")
...
pred_swim <- predict.gam(model_swim_1, newdata = grid_swim, type = "link", se.fit = TRUE, exclude = "s(DATE)")
...
inverse_link <- family(model_swim_1)$linkinv
